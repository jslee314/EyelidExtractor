...
public:
	...
	{
		// ASSERT 메크로를 이용하여 픽셀 데이터형의 크기가 1byte인 경우에만 동작할 수 있도록 한다.
		ASSERT(sizeof(T) == 1); // CMyImage 클래스는 템플릿을 통해 정의되어있지만, 영상 파일 입출력은 픽셀데이터가  BYTE형의 경우만 가능
		...
		if (strcmp(".BMP", &filename[strlen(filename) - 4]))
		{
			...
			// 256레벨의 회색조 영상(8bit), 혹은 트루컬러 영상만 읽을 수 있도록 한다(24bit) 
	...
	/*
	CMyImage(void);											// 1. 기본 생성자
	CMyImage(int nWidth, int nHeight, int nChannels = 1);	// 2. 재정의된 생성자
	CMyImage(const CMyImage& myImage);						// 3. 복사 생성자
	~CMyImage(void);										// 4. 소멸자
	// 5. 대입 연산자
	CMyImage& operator=(const CMyImage& myImage) {
	...
	template <typename From>
	CMyImage(const CMyImage<From>& myImage);				// 6. 형변환 생성자
	inline T& GetAt(int x, int y, int c = 0) const;			// 7. 픽셀 참조자
	void SetConstValue(T val);								// 8. 영상 초기화 
	bool IsEmpty() const;									// 9. 영상이 있는지 여부 
