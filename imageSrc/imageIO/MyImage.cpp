...
// 2. 재정의된 생성자
template <typename T>
CMyImage<T>::CMyImage(int nWidth, int nHeight, int nChannels = 1)
...
// 3. 복사 생성자
template <typename T>
CMyImage<T>::CMyImage(const CMyImage& myImage)
{
...
// 4. 소멸자
template <typename T>CMyImage<T>::~CMyImage(void)
{
...
CMyImage<T>::CMyImage(const CMyImage<From>& myImage)
{
...
// 7. 픽셀 참조자
template <typename T>
inline T& CMyImage<T>::GetAt(int x, int y, int c = 0) const {
...
// 8. 영상 초기화
template <typename T>
void CMyImage<T>::SetConstValue(T val)
{
	if (val == 0)   // 조건 (1)
	{
...
template <typename T>
bool CMyImage<T>::LoadImage(const char* filename)
{
...
template <typename T>
bool CMyImage<T>::SaveImage(const char* filename)
{
	ASSERT(sizeof(T) == 1); // BYTE형의 경우만 가능
...
template <typename T>
void CMyImage<T>::PutChannelImg(const CMyImage<T>& src, int nChannel)
{
...
template <typename T>
CMyImage<T> CMyImage<T>::GetChannelImg(int nChannel) const
{
...
template <typename T>
CMyImage<T> CMyImage<T>::Transpose() const
{
	// 반환할 결과 영상 생성
...
template <typename T>
BYTE CMyImage<T>::NearestNeighbor(double x, double y)
{
...
template <typename T>
BYTE CMyImage<T>::BiLinearIntp(double x, double y)
{
...
template <typename T>
BYTE CMyImage<T>::BiCubicIntp(double x, double y)
{
...
template <typename T>
BYTE CMyImage<T>::CubicConv(double x, double y)
{
	int px[4], py[4];
